...
Incluye lectura USB de Arduino Nano para 3 EZO-pH y 1 EZO-RTD.
"""
...
# ===== Arduino Nano / Atlas EZO por USB serial =====
NANO_SERIAL_PORT = os.environ.get("NANO_SERIAL_PORT", "").strip()
NANO_SERIAL_BAUD = parse_int(os.environ.get("NANO_SERIAL_BAUD", "115200"), 115200)
NANO_BOOT_WAIT_SEC = float(os.environ.get("NANO_BOOT_WAIT_SEC", "4.0"))
NANO_STALE_SEC = float(os.environ.get("NANO_STALE_SEC", "8.0"))
NANO_RECONNECT_SEC = float(os.environ.get("NANO_RECONNECT_SEC", "2.0"))
NANO_SIMULADOR = os.environ.get("NANO_SIMULADOR", "").strip().lower() in {"1", "true", "yes"}
PH_KEY_BY_FERMENTER = {"F1": "ph1", "F2": "ph2", "F3": "ph3"}
...
# ===== Arduino Nano Atlas EZO por USB serial =====
class NanoSerialReader:
    """Lector no bloqueante del Nano.

    El Nano envía una línea JSON por muestra. Esta clase detecta automáticamente
    /dev/ttyUSB* y /dev/ttyACM*, reconecta si se desconecta y mantiene la última
    muestra en memoria para que Tkinter nunca espere por el puerto serial.
    """

    VALUE_KEYS = ("ph1", "ph2", "ph3", "rtd")

    def __init__(self):
        self.explicit_port = NANO_SERIAL_PORT
        self.baud = NANO_SERIAL_BAUD
        self.sim = NANO_SIMULADOR
        ...
        self._state = {
            "connected": False,
            "port": None,
            "status": "Inicializando lector Nano USB",
            "sample": None,
        }
        self._thread = threading.Thread(target=self._run, name="NanoSerialReader", daemon=True)
        self._thread.start()
    ...
    def _normalize_sample(self, payload, port):
        values = {key: self._safe_float(payload.get(key)) for key in self.VALUE_KEYS}
        sample = {
            **values,
            "seq": self._safe_int(payload.get("seq")),
            "nano_ms": self._safe_int(payload.get("ms", payload.get("nano_ms"))),
            "port": port,
            "received_at": now(),
        }
        ...
        for key in self.VALUE_KEYS:
            ok_key = f"ok_{key}"
            code_key = f"code_{key}"
            sample[ok_key] = self._safe_bool(payload.get(ok_key), fallback=values[key] is not None)
            sample[code_key] = self._safe_int(payload.get(code_key))

        self._sample_counter += 1
        sample["sample_id"] = self._sample_counter
        missing = [key.upper() for key in self.VALUE_KEYS if not sample[f"ok_{key}"]]
        sample["status"] = "OK" if not missing else "Parcial: " + ", ".join(missing)
        return sample
    ...
    def _candidate_ports(self):
        ...
        for port in list_ports.comports():
            ...
            is_usb_device = device.startswith("/dev/ttyUSB") or device.startswith("/dev/ttyACM")
            looks_like_nano = any(
                token in description
                for token in ("arduino", "ch340", "ch341", "usb serial", "cp210", "ftdi")
            )
            if is_usb_device or looks_like_nano:
    ...
    def _run_simulator(self):
        ...
        while not self._stop.is_set():
            ...
            payload = {
                "seq": int(elapsed / 2.0),
                "ms": int(elapsed * 1000),
                "ph1": 7.00 + 0.08 * math.sin(elapsed / 17.0),
                "ph2": 6.80 + 0.10 * math.sin(elapsed / 21.0),
                "ph3": 7.20 + 0.07 * math.sin(elapsed / 25.0),
                "rtd": 24.0 + 0.5 * math.sin(elapsed / 45.0),
                "ok_ph1": 1,
                "ok_ph2": 1,
                "ok_ph3": 1,
                "ok_rtd": 1,
            ...
            sample = self._normalize_sample(payload, "SIMULADOR")
            self._last_sample_monotonic = time.monotonic()
            self._set_state(connected=True, port="SIMULADOR", status="Nano simulado OK", sample=sample)
            self._stop.wait(2.0)
    ...
    def _run(self):
        ...
        try:
            ...
            self._set_state(
                connected=False,
                status=f"pyserial no instalado: {exc}",
                port=None,
            )
            return
        ...
        while not self._stop.is_set():
            ...
            if not ports:
                detail = self.explicit_port or "/dev/ttyUSB* o /dev/ttyACM*"
                self._set_state(
                    connected=False,
                    port=self.explicit_port or None,
                    status=f"Nano USB no encontrado ({detail})",
                )
                self._stop.wait(NANO_RECONNECT_SEC)
                continue
            ...
            for port in ports:
                ...
                valid_seen = False
                try:
                    self._set_state(connected=False, port=port, status=f"Abriendo {port}...")
                    ser = serial.Serial(port=port, baudrate=self.baud, timeout=1.0, write_timeout=1.0)
                    self._serial = ser
                    self._force_reconnect.clear()
                    self._set_state(connected=True, port=port, status=f"Esperando arranque Nano en {port}")

                    # Abrir el puerto normalmente reinicia el Nano mediante DTR.
                    boot_deadline = time.monotonic() + max(0.0, NANO_BOOT_WAIT_SEC)
                    while time.monotonic() < boot_deadline and not self._stop.is_set():
                        if self._force_reconnect.is_set():
                            raise RuntimeError("Reconexión solicitada")
                        time.sleep(0.05)
                    try:
                    ...
                    while not self._stop.is_set():
                        ...
                        raw = ser.readline()
                        if not raw:
                            if time.monotonic() - last_valid > max(15.0, NANO_STALE_SEC * 2.0):
                                raise TimeoutError("Nano conectado, pero sin datos JSON")
                            continue
                        ...
                        if not isinstance(payload, dict):
                            continue
                        sample = self._normalize_sample(payload, port)
                        valid_seen = True
                        last_valid = time.monotonic()
                        self._last_sample_monotonic = last_valid
                        self._set_state(
                            connected=True,
                            port=port,
                    ...
                    self._set_state(connected=False, port=port, status=f"{port}: {exc}")
                finally:
                    self._force_reconnect.clear()
                    if self._serial is not None:
                        try:
                            ...
                            pass
                    self._serial = None
                # Si este puerto entregó JSON, se prioriza en la próxima reconexión.
                # Si abrió pero nunca entregó JSON, probamos el siguiente USB serial.
                if valid_seen or self.explicit_port:
                    break
    ...
    def snapshot(self):
        with self._lock:
            ...
            sample = self._state.get("sample")
            state["sample"] = dict(sample) if isinstance(sample, dict) else None
        if self._last_sample_monotonic is None:
            state["age_sec"] = None
        else:
            state["age_sec"] = max(0.0, time.monotonic() - self._last_sample_monotonic)
        return state

    def request_reconnect(self):
        self._set_state(status="Reconexión Nano solicitada")
...
class LightFermenterPanel(ttk.Frame):
    def __init__(self, parent, app, name, hw: Hardware, backup_path_getter):
        ...
        self.ph_str = tk.StringVar(value="--")
        ...
        self.pending_mode = None          # None, "cold", "hot"
        self.switch_lock_until = None     # datetime hasta cuando mantener todo cerrado
        self.min_off_between_sides = 20.0 # s, partir con 20?60 s
        self.min_hold_seconds = 60.0      # s, tiempo minimo en un lado antes de invertir
    ...
    def _build_ui(self):
        ...
        meas.columnconfigure((0, 1, 2, 3, 4), weight=1)

        ttk.Label(meas, text="T DS18B20 (°C)").grid(row=0, column=0, sticky="w")
        ...
        ttk.Label(meas, text="pH Atlas").grid(row=0, column=2, sticky="w")
        ttk.Label(meas, textvariable=self.ph_str, font=("Segoe UI", 16, "bold")).grid(
            row=1, column=2, sticky="w", pady=(0, 6)
        )

        ttk.Label(meas, text="SP (°C)").grid(row=0, column=3, sticky="w")
        sp_entry = ttk.Entry(meas, textvariable=self.sp_entry, width=8, justify="center")
        sp_entry.grid(row=1, column=3, sticky="w")
        sp_entry.bind("<Return>", lambda _e: self.apply_sp())
        ttk.Button(meas, text="Aplicar", command=self.apply_sp).grid(row=2, column=3, sticky="w", pady=(2, 6))

        ttk.Label(meas, text="Banda (°C)").grid(row=0, column=4, sticky="w")
        band_entry = ttk.Entry(meas, textvariable=self.band_entry, width=8, justify="center")
        band_entry.grid(row=1, column=4, sticky="w")
        band_entry.bind("<Return>", lambda _e: self.apply_band())
        ttk.Button(meas, text="Aplicar", command=self.apply_band).grid(row=2, column=4, sticky="w", pady=(2, 6))
    ...
    def update_process(self):
        ...
        ph_value, ph_ok, ph_fresh = self.app.get_ezo_ph_for_fermenter(self.name)
        if not ph_fresh:
            self.ph_str.set("--")
        elif ph_ok and ph_value is not None:
            self.ph_str.set(f"{ph_value:.3f}")
        else:
            self.ph_str.set("ERR")
        ...
        if not self.manual_mode.get():
            ...
            # Estado actual
            if self.cold_in and not self.hot_in:
                current_mode = "cold"
            elif self.hot_in and not self.cold_in:
                current_mode = "hot"
            else:
                current_mode = "off"
            ...
            time_in_mode = (tnow - self.last_mode_change).total_seconds()

            # 1) Si estamos en ventana de bloqueo entre lados, mantener TODO cerrado
            if self.switch_lock_until is not None and tnow < self.switch_lock_until:
                self.cold_in = False
                self.hot_in = False

            else:
                # 2) Si terminó el bloqueo, habilitar modo pendiente si existe
                if self.switch_lock_until is not None and tnow >= self.switch_lock_until:
                    self.switch_lock_until = None
                    if self.pending_mode in ("cold", "hot"):
                        target_mode = self.pending_mode
                    self.pending_mode = None

                # 3) Aplicar lógica de control
                if target_mode == "off":
                    # Dentro de banda: cerrar todo
                    if current_mode != "off":
                        self.cold_in = False
                        self.hot_in = False
                        self.last_mode_change = tnow

                elif target_mode == current_mode:
                    # Ya estamos en el lado correcto: no hacer nada
                    pass

                elif current_mode == "off":
                    # Desde OFF a un lado: abrir directo
                    self.cold_in = (target_mode == "cold")
                    self.hot_in = (target_mode == "hot")
                    self.last_mode_change = tnow

                else:
                    # Inversión cold <-> hot:
                    # exigir permanencia mínima antes de permitir cambio
                    if time_in_mode >= self.min_hold_seconds:
                        self.cold_in = False
                        self.hot_in = False
                        self.pending_mode = target_mode
                        self.switch_lock_until = tnow + dt.timedelta(seconds=self.min_off_between_sides)
                        self.last_mode_change = tnow
                    else:
                        # Aún no cumple permanencia mínima: mantener estado actual
                        pass
...
class EZOPlotWindow:
    def __init__(self, app):
        self.app = app
        self.top = tk.Toplevel(app)
        self.top.title("pH + temperatura RTD Atlas – Nano USB")
        self.current_window_hours = None
        self._loading = False
        self._load_id = 0
        self._refresh_job = None
        ...
        def set_window(hours):
            self.current_window_hours = hours
            self.request_data()
        ...
        self.status = ttk.Label(self.top, text="Cargando datos pH/RTD...")
        self.status.pack(anchor="w", padx=8, pady=4)
        ...
        self.fig, (self.ax_ph, self.ax_rtd) = plt.subplots(
            2,
            1,
            sharex=True,
            figsize=(9, 6),
        ...
        self.canvas = FigureCanvasTkAgg(self.fig, master=self.top)
        self.canvas.get_tk_widget().pack(fill="both", expand=True)
        self.ax_rtd.xaxis.set_major_formatter(mdates.DateFormatter("%m-%d %H:%M"))
    ...
    def close(self):
        ...
        if self.top in self.app._plot_windows:
            self.app._plot_windows.remove(self.top)
        self.app._ezo_plot_window = None
        self.top.destroy()
    ...
    def request_data(self):
        ...
        req_id = self._load_id
        days_window = 3650 if self.current_window_hours is None else (self.current_window_hours / 24.0)
        path = self.app.get_ezo_backup_path()

        def worker():
            data = self.app._read_recent_ezo_backup(days=days_window, path=path)
            self.app.after(0, lambda: self._on_data(req_id, data))
    ...
    def _on_data(self, req_id, data):
        ...
        ts = data.get("ts", []) if data else []
        if not ts:
            self.status.config(text="Sin datos recientes en el backup pH/RTD.")
        else:
            ph_specs = (("ph1", "F1 pH"), ("ph2", "F2 pH"), ("ph3", "F3 pH"))
            all_ph = []
            for key, label in ph_specs:
                values = data.get(key, [])
                self.ax_ph.plot(ts, values, label=label)
                all_ph.extend(v for v in values if math.isfinite(v))

            rtd_values = data.get("rtd", [])
            self.ax_rtd.plot(ts, rtd_values, label="RTD Atlas")
            all_rtd = [v for v in rtd_values if math.isfinite(v)]

            self.ax_ph.set_title("pH Atlas vs. tiempo")
            self.ax_ph.set_ylabel("pH")
            self.ax_ph.legend(loc="upper left")
            if all_ph:
                pmin, pmax = min(all_ph), max(all_ph)
                pad = (pmax - pmin) * 0.10 if pmax != pmin else 0.2
                self.ax_ph.set_ylim(pmin - pad, pmax + pad)

            self.ax_rtd.set_title("Temperatura RTD Atlas vs. tiempo")
            self.ax_rtd.set_ylabel("°C")
            self.ax_rtd.set_xlabel("Fecha y hora")
            self.ax_rtd.legend(loc="upper left")
            if all_rtd:
                tmin, tmax = min(all_rtd), max(all_rtd)
                pad = (tmax - tmin) * 0.10 if tmax != tmin else 0.5
                self.ax_rtd.set_ylim(tmin - pad, tmax + pad)

            if self.current_window_hours is None:
                left, right = min(ts), max(ts)
                if left == right:
                    left = right - dt.timedelta(minutes=1)
                self.ax_ph.set_xlim(left, right)
                self.status.config(text="Fuente: backup global pH/RTD (tiempo real)")
            else:
                right = now()
                left = right - dt.timedelta(hours=self.current_window_hours)
                self.ax_ph.set_xlim(left, right)
                if self.current_window_hours >= 24:
                    ...
                    rango = f"últimas {self.current_window_hours} horas"
                self.status.config(text=f"Fuente: backup global pH/RTD ({rango})")
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Lector USB del Arduino Nano (3 pH + 1 RTD). Corre en hilo separado.
        self.nano_reader = NanoSerialReader()
        self.ezo_latest_sample = None
        self.ezo_data_fresh = False
        self._ezo_last_logged_id = None
        self._ezo_last_write_error = None
        self.nano_status_var = tk.StringVar(value="Nano USB: buscando...")
        self.nano_rtd_var = tk.StringVar(value="RTD Atlas: --.- °C")
        self.nano_last_var = tk.StringVar(value="Última muestra: --")
        ...
        nano_row = ttk.Frame(top_bar)
        nano_row.grid(row=2, column=0, sticky="w", pady=(4, 0))
        self.nano_led = Led(nano_row, size=18)
        self.nano_led.widget().grid(row=0, column=0, padx=(0, 6))
        ttk.Label(nano_row, textvariable=self.nano_status_var, font=("Segoe UI", 10, "bold")).grid(
            row=0, column=1, sticky="w"
        )
        ttk.Label(nano_row, text=" | ").grid(row=0, column=2)
        ttk.Label(nano_row, textvariable=self.nano_rtd_var, font=("Segoe UI", 10, "bold")).grid(
            row=0, column=3, sticky="w"
        )
        ttk.Label(nano_row, text=" | ").grid(row=0, column=4)
        ttk.Label(nano_row, textvariable=self.nano_last_var).grid(row=0, column=5, sticky="w")

        if self.logo_img is not None:
            logo_label = ttk.Label(top_bar, image=self.logo_img)
            logo_label.grid(row=0, column=1, rowspan=3, sticky="e", padx=(0, 10))
        ...
        self.backup_ezo_path = tk.StringVar(value=os.path.abspath("./Backup/backup_global_ph_rtd.csv"))
        ttk.Label(backup_frame, text="Backup global pH/RTD Nano (automático):").grid(
            row=2, column=0, sticky="w", pady=(4, 0)
        )
        ttk.Entry(backup_frame, textvariable=self.backup_ezo_path).grid(
            row=2, column=1, sticky="ew", padx=(4, 4), pady=(4, 0)
        )
        ttk.Button(backup_frame, text="Destino…", command=self.pick_backup_ezo).grid(
            row=2, column=2, padx=(4, 0), pady=(4, 0)
        )

        backup_btns = ttk.Frame(backup_frame)
        backup_btns.grid(row=3, column=0, columnspan=3, sticky="w", pady=(6, 0))
        ttk.Button(backup_btns, text="Borrar backup temperatura", command=self.clear_backup_temp).pack(
            side="left", padx=(0, 6)
        )
        ttk.Button(backup_btns, text="Borrar backup CO2", command=self.clear_backup_co2).pack(
            side="left", padx=(0, 6)
        )
        ttk.Button(backup_btns, text="Borrar backup pH/RTD", command=self.clear_backup_ezo).pack(
            side="left", padx=(0, 6)
        )
        ttk.Button(backup_btns, text="Gráfico pH + RTD", command=self.open_ezo_plot).pack(
            side="left", padx=(0, 6)
        )
        ttk.Button(backup_btns, text="Reconectar Nano USB", command=self.nano_reader.request_reconnect).pack(
            side="left"
        )
    ...
    def get_ezo_backup_path(self):
        path = self.backup_ezo_path.get().strip()
        if not path:
            path = os.path.abspath("./Backup/backup_global_ph_rtd.csv")
            self.backup_ezo_path.set(path)
        if not path.lower().endswith(".csv"):
            path += ".csv"
    ...
    def pick_backup_ezo(self):
        fn = filedialog.asksaveasfilename(
            title="Elegir archivo de BACKUP global pH/RTD",
            initialfile=os.path.basename(self.backup_ezo_path.get()),
            defaultextension=".csv",
            filetypes=[("CSV", "*.csv"), ("Todos", "*.*")],
        )
        _restore_focus(self)
        if fn:
            self.backup_ezo_path.set(fn)
            os.makedirs(os.path.dirname(fn) or ".", exist_ok=True)

    def clear_backup_ezo(self):
        path = self.get_ezo_backup_path()
        if not os.path.exists(path):
            messagebox.showinfo("Backup pH/RTD", f"No existe:\n{path}")
            return
        if not messagebox.askyesno("Backup pH/RTD", f"¿Borrar backup?\n{path}"):
            return
        try:
            os.remove(path)
            messagebox.showinfo("Backup pH/RTD", f"Backup borrado:\n{path}")
        except Exception as exc:
            messagebox.showerror("Backup pH/RTD", f"No se pudo borrar.\n{exc}")

    @staticmethod
    def _ezo_csv_float(value):
        if value is None:
            return ""
        try:
            value = float(value)
    ...
    def _ezo_backup_write_row(self, sample):
        path = self.get_ezo_backup_path()
        row = {
            "timestamp": sample["received_at"].strftime("%Y-%m-%d %H:%M:%S"),
            "port": sample.get("port") or "",
            "seq": "" if sample.get("seq") is None else sample.get("seq"),
            "nano_ms": "" if sample.get("nano_ms") is None else sample.get("nano_ms"),
            "ph_F1": self._ezo_csv_float(sample.get("ph1")) if sample.get("ok_ph1") else "",
            "ph_F2": self._ezo_csv_float(sample.get("ph2")) if sample.get("ok_ph2") else "",
            "ph_F3": self._ezo_csv_float(sample.get("ph3")) if sample.get("ok_ph3") else "",
            "rtd_C": self._ezo_csv_float(sample.get("rtd")) if sample.get("ok_rtd") else "",
            "ok_F1": int(bool(sample.get("ok_ph1"))),
            "ok_F2": int(bool(sample.get("ok_ph2"))),
            "ok_F3": int(bool(sample.get("ok_ph3"))),
            "ok_RTD": int(bool(sample.get("ok_rtd"))),
            "code_F1": "" if sample.get("code_ph1") is None else sample.get("code_ph1"),
            "code_F2": "" if sample.get("code_ph2") is None else sample.get("code_ph2"),
            "code_F3": "" if sample.get("code_ph3") is None else sample.get("code_ph3"),
            "code_RTD": "" if sample.get("code_rtd") is None else sample.get("code_rtd"),
        ...
        try:
            ...
            error_text = str(exc)
            if error_text != self._ezo_last_write_error:
                print(f"[EZO] Error escribiendo backup {path}: {exc}")
                self._ezo_last_write_error = error_text

    def _read_recent_ezo_backup(self, days=10, path=None):
        if path is None:
            path = self.get_ezo_backup_path()
        if not os.path.exists(path):
            return {}
        cutoff = now() - dt.timedelta(days=days)
        data = {"ts": [], "ph1": [], "ph2": [], "ph3": [], "rtd": []}
            ...
            with open(path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M:%S"):
                        try:
                            timestamp = dt.datetime.strptime(ts_raw, fmt)
                            break
                        except Exception:
                            continue
                    if timestamp is None or timestamp < cutoff:
                        continue
                    data["ts"].append(timestamp)
                    data["ph1"].append(read_float(row, "ph_F1"))
                    data["ph2"].append(read_float(row, "ph_F2"))
                    data["ph3"].append(read_float(row, "ph_F3"))
                    data["rtd"].append(read_float(row, "rtd_C"))
        except Exception as exc:
            print(f"[EZO] Error leyendo backup {path}: {exc}")
            return {}
        return data

    def get_ezo_ph_for_fermenter(self, fermenter):
        key = PH_KEY_BY_FERMENTER.get(fermenter)
        sample = self.ezo_latest_sample
        if key is None or not self.ezo_data_fresh or not sample:
            return None, False, False
        return sample.get(key), bool(sample.get(f"ok_{key}")), True

    def _ezo_tick(self):
        snapshot = self.nano_reader.snapshot()
        sample = snapshot.get("sample")
        age = snapshot.get("age_sec")
        connected = bool(snapshot.get("connected"))
        fresh = bool(sample) and connected and age is not None and age <= NANO_STALE_SEC
        self.ezo_data_fresh = fresh

        if sample:
            self.ezo_latest_sample = sample
            sample_id = sample.get("sample_id")
            if sample_id is not None and sample_id != self._ezo_last_logged_id:
                self._ezo_backup_write_row(sample)
                self._ezo_last_logged_id = sample_id

        port = snapshot.get("port") or "sin puerto"
        status = snapshot.get("status") or "Sin estado"

        if fresh and sample:
            ok_all = all(bool(sample.get(f"ok_{key}")) for key in ("ph1", "ph2", "ph3", "rtd"))
            self.nano_led.set_color("#22c55e" if ok_all else "#eab308")
            self.nano_status_var.set(f"Nano USB {port}: {status}")
            rtd = sample.get("rtd")
            if sample.get("ok_rtd") and rtd is not None:
                self.nano_rtd_var.set(f"RTD Atlas: {rtd:.3f} °C")
            else:
                self.nano_rtd_var.set("RTD Atlas: ERR")
            received_at = sample.get("received_at")
            if isinstance(received_at, dt.datetime):
                self.nano_last_var.set(f"Última muestra: {received_at.strftime('%H:%M:%S')}")
        elif connected:
            self.nano_led.set_color("#eab308")
            self.nano_status_var.set(f"Nano USB {port}: {status}")
            self.nano_rtd_var.set("RTD Atlas: --.- °C")
            self.nano_last_var.set("Última muestra: esperando datos")
        else:
            self.nano_led.set_color("#ef4444")
            self.nano_status_var.set(f"Nano USB: {status}")
            self.nano_rtd_var.set("RTD Atlas: --.- °C")
            self.nano_last_var.set("Última muestra: --")
    ...
    def open_ezo_plot(self):
        mpl_spec = importlib_util.find_spec("matplotlib")
        if not mpl_spec:
            messagebox.showerror("Gráfico", "Instala matplotlib para usar el gráfico pH/RTD.")
            return
        if self._ezo_plot_window is not None and self._ezo_plot_window.winfo_exists():
            try:
                self._ezo_plot_window.lift()
                self._ezo_plot_window.focus_force()
            except Exception:
                pass
            return
        window = EZOPlotWindow(self)
        self._ezo_plot_window = window.top
